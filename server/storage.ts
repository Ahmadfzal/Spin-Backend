...
export interface IStorage {
  ...
  updateUserBalance(coinDelta: number, freeSpinDelta: number): Promise<User>;
  topUp(amount: number): Promise<User>;
  withdraw(amount: number): Promise<User>;
}
...
export class DatabaseStorage implements IStorage {
  async getUser(): Promise<User> {
    ...
    if (!user) {
      const [newUser] = await db.insert(users).values({ coins: 10000, freeSpins: 0 }).returning();
      return newUser;
    }
    return user;
  }
  ...
  async getSpins(): Promise<Spin[]> {
    return await db.select().from(spins).orderBy(desc(spins.createdAt)).limit(10);
  }
}
